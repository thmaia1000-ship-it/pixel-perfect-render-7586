...
function safeLocalStorage() {
  try {
    ...
    // Fallback if localStorage access is forbidden
  }
  return {
...
export function brokeredPreviewStorage() {
  ...
  if (!projectId || !framed) return localStore;
  ...
  return {
    ...
    setItem: (key: string, value: string) => {
      ...
      return request("lovable-preview-auth:set", key, value).then((res) => {
        if (res && res.ok && typeof res.value === "string" && localStore.getItem(key) === value) {
          if (res.value === "") localStore.removeItem(key);
          else localStore.setItem(key, res.value);
        }
      });
    },
